...
<h3 class="preview__header" itemprop="name">{{ page.title }}</h3>
<span class="preview__date" itemprop="datePublished" datetime="{{ page.date | date_to_xmlschema }}">{{ page.date | date: "%Y년 %-m부터 약 %-d개월 이내" }}</span>
<p class="preview__excerpt" itemprop="description">{{ page.categories }}</p>
<p class="preview__excerpt" itemprop="description">{{ page.description }}</p>
