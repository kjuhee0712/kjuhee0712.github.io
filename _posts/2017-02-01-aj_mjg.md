...
<h3 class="preview__header" itemprop="name">{{ post.title }}</h3>
<span class="preview__date" itemprop="datePublished" datetime="{{ post.date | date_to_xmlschema }}">{{ post.date | date: "%Y년 %-m부터 약 %-d개월 이내" }}</span>
<p class="preview__excerpt" itemprop="description">{{ post.categories }}</p>
